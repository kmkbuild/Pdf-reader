...
export default function App() {
    ...
    (async () => {
      try {
        ...
        const [bm, nt, lr, rb, perm] = await Promise.all([
          dbGet("bookmarks"), dbGet("notes"), dbGet("lastRead"),
          dbGet("recentBooks"), dbGet("permState"),
        ]);
        ...
        setTimeout(() => setShowPerm(true), 1000);
      }
    })();
  }, []);
  ...
  const touchControls = () => {
    ...
    controlsTimer.current = setTimeout(() => {
      if (!activePanel) setShowControls(false);
    }, 4000);
  };
  ...
  const grantPermission = async () => {
    ...
    try {
      ...
      await dbSet("permState", "granted");
      setScanning(true); setScanCount(0); setScanStatus("Starting scan…");
      ...
      const onFound = async (book) => {
        ...
        setLibrary(prev => {
          if (prev.find(b => b.name === book.name)) return prev;
          return [...prev, book];
        });
      };
      await scanDir(dir, 0, onFound);
      setScanning(false); setScanStatus("");
      if (count === 0) toast_("No new PDFs found. Try selecting Internal Storage root.");
      else toast_(`Found ${count} PDF${count > 1 ? "s" : ""} on your device!`, "success");
    } catch (e) {
      setScanning(false); setScanStatus("");
      if (e?.name !== "AbortError") toast_("Could not access storage.");
      setShowPerm(false);
    }
  };
      ...
      try {
        ...
        while (idx !== -1) {
          ...
          if (snippets.length < 3) {
            // Build snippet with highlighted term
        ...
        if (snippets.length) {
          totalMatches += count;
          results.push({ page: p, snippets, count });
        }
      } catch {}
    }
    setSearchRes(results);
    setSearchTotal(totalMatches);
    setSearching(false);
  ...
  const startVoice = async () => {
    ...
    } catch { toast_("Text-to-speech not available on this device."); }
  };
  const stopVoice = () => {
    ...
    <div style={{ height: "100vh", background: C.bg0, color: C.tx,
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 290,
          ...
          <div style={{ background: "linear-gradient(135deg,#0c1428,#101e38)",
            ...
            <div style={{ marginTop: 12, fontSize: 11, color: C.txD }}>
              Books appear in your library as they're found
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 300,
          ...
          <div style={{ background: "linear-gradient(135deg,#0c1428,#101e38)",
            ...
            <div style={{ fontSize: 13, color: C.txM, lineHeight: 1.8, textAlign: "center", marginBottom: 20 }}>
              To find all PDFs on your phone, tap below then:
            </div>
            {[
              { n: "1", t: "A folder picker will open" },
              { n: "2", t: 'Select "Internal Storage" (the root)' },
              { n: "3", t: "Tap Allow / Select" },
              { n: "4", t: "App scans all folders automatically" },
            ].map((s, i) => (
            ...
            <div style={{ height: 20 }} />
            <GlowBtn label="📂 Open Storage & Scan All PDFs" onClick={grantPermission} full />
            <div style={{ height: 10 }} />
            <GlowBtn label="Pick PDFs Manually" onClick={() => { setShowPerm(false); skipPerm(); fileInputRef.current.click(); }} full outline />
      ...
      <div style={{ position: "fixed", top: 0, left: 0, bottom: 0, width: 300, zIndex: 90,
        ...
        <div style={{ padding: "24px 20px 16px", borderBottom: `1px solid ${C.border}` }}>
          ...
          <div style={{ fontSize: 12, color: C.txM, marginTop: 4 }}>
            {library.length} books in library
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "12px 8px" }}>
          {[
            ...
            { icon: "📂", label: "Scan Storage", sub: "Find all PDFs automatically", action: async () => { setDrawerOpen(false); await grantPermission(); } },
            ...(screen === "reader" ? [
              null, // divider
              ...
              { icon: "🔍", label: "Search PDF", sub: "Full text search with highlights", action: () => { setActivePanel("search"); setDrawerOpen(false); } },
              { icon: "🎧", label: "Voice Reader", sub: "Listen to this page", action: () => { setActivePanel("voice"); setDrawerOpen(false); } },
              { icon: "✏️", label: "Notes", sub: `${Object.keys(bNotes).length} notes`, action: () => { setActivePanel("notes"); setDrawerOpen(false); } },
              { icon: "📑", label: "Contents", sub: toc.length > 0 ? `${toc.length} sections` : "Not available", action: () => { setActivePanel("toc"); setDrawerOpen(false); } },
            ] : []),
          ].map((item, i) =>
            item === null
        ...
        {/* Reading progress in drawer (reader only) */}
      ...
      <div style={{ flex: 1, overflow: "hidden", position: "relative", zIndex: 1 }}>
        ...
        {tab === "home" && (
          <div style={{ height: "100%", overflowY: "auto", padding: "0 0 90px" }}>
            ...
            <div style={{ padding: "20px 20px 0",
              ...
              <button onClick={() => setDrawerOpen(true)}
                style={{ background: "rgba(255,255,255,0.06)",
                  ...
                  width: 40, height: 40, cursor: "pointer", color: C.tx,
            ...
            <div style={{ padding: "22px 20px 0" }}>
              <div style={{ fontSize: 13, color: C.txM, marginBottom: 4 }}>
                {getGreeting()}
              </div>
            ...
            {library.length === 0 ? (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center",
                justifyContent: "center", minHeight: "55vh", gap: 16,
        ...
        {tab === "reader" && screen === "reader" && (
          <div style={{ height: "100%", position: "relative", background: "#0f0f1a" }}>
            ...
            <div ref={containerRef}
              ...
              <div style={{ borderRadius: 8, overflow: "hidden",
                boxShadow: `0 0 60px rgba(37,99,235,0.12), 0 32px 80px rgba(0,0,0,0.8)` }}>
            ...
            <div style={{ position: "absolute", top: 0, left: 0, right: 0,
              background: "linear-gradient(180deg,rgba(7,13,26,0.96),transparent)",
                ...
                <div style={{ padding: "0 20px 100px" }}>
                  ...
                  {activePanel === "search" && (
                    <>
                      ...
                      {searchRes.length > 0 && (
                        <div style={{ display: "flex", alignItems: "center",
                          ...
                          <div style={{ fontSize: 12, color: C.neonL, fontWeight: 700 }}>
                            {searchTotal} match{searchTotal !== 1 ? "es" : ""} on {searchRes.length} page{searchRes.length !== 1 ? "s" : ""}
                          </div>
                      ...
                      {searching && (
                        <div style={{ textAlign: "center", color: C.neonL, padding: 16, fontSize: 13 }}>
                          🔍 Searching {numPages} pages…
                        </div>
                      )}
                      ...
                      {searchRes.map((r, i) => (
                        <div key={i}
                          onClick={() => { goToPage(r.page); setCurrentResultIdx(i); }}
                          style={{ ...glassCard(r.page === currentPage && i === currentResultIdx),
                            marginBottom: 10, cursor: "pointer",
                            transition: "all .2s" }}
                          onMouseEnter={e => e.currentTarget.style.borderColor = C.borderG}
                          onMouseLeave={e => e.currentTarget.style.borderColor = r.page === currentPage ? C.borderG : C.border}>
                          <div style={{ display: "flex", justifyContent: "space-between",
                          ...
                          {r.snippets.map((s, j) => (
                            <div key={j} dir="auto"
                              style={{ fontSize: 12, color: C.txM, lineHeight: 1.6,
                                marginBottom: j < r.snippets.length - 1 ? 6 : 0 }}>
                              …{s.before}
                              <span style={{ background: "rgba(59,130,246,0.3)",
                                color: C.white, borderRadius: 3, padding: "0 2px",
                                fontWeight: 700 }}>
                                {s.match}
                              </span>
                              {s.after}…
                  ...
                  {activePanel === "voice" && (
                    <>
                      ...
                      <div style={{ fontSize: 12, color: C.txM, marginBottom: 20 }}>
                        Listening to Page {currentPage}
                      </div>

                      {/* Waveform */}
                      <div style={{ display: "flex", alignItems: "center",
                        justifyContent: "center", gap: 3, margin: "16px 0", height: 64 }}>
                      ...
                      <div style={{ display: "flex", justifyContent: "center",
                        ...
                        <button onClick={() => goToPage(currentPage - 1)}
                          ...
                          style={{ ...glassBtn({ width: 44, height: 44 }),
                            opacity: currentPage === 1 ? 0.3 : 1 }}>⏮</button>
                        ...
                        <button onClick={() => { stopVoice(); goToPage(currentPage + 1); }}
                          disabled={currentPage === numPages}
                          style={{ ...glassBtn({ width: 44, height: 44 }),
                            opacity: currentPage === numPages ? 0.3 : 1 }}>⏭</button>
                      </div>
                  ...
                  {activePanel === "saved" && (
                    <>
                      ...
                      <GlowBtn
                        label={isBm ? "Remove from Saved" : `Save Page ${currentPage}`}
                        onClick={toggleBm} outline={isBm} />
                      <div style={{ height: 16 }} />
                      {bBms.length === 0 ? (
                        <div style={{ textAlign: "center", color: C.txM, padding: "20px 0", fontSize: 14 }}>
                          No saved pages yet.
                        </div>
                      ) : bBms.map(pg => (
                        <div key={pg} onClick={() => { goToPage(pg); setActivePanel(null); }}
                          ...
                          <div>
                            <div style={{ fontSize: 14, fontWeight: 700,
                              color: pg === currentPage ? C.neonL : C.tx }}>
                              Page {pg}
                            </div>
                            {bNotes[pg] && (
                          ...
                          <button onClick={e => { e.stopPropagation();
                            setBookmarks(p => ({ ...p, [bKey]: bBms.filter(x => x !== pg) })); }}
                            style={{ background: "none", border: "none",
                              cursor: "pointer", color: C.txM, fontSize: 20 }}>×</button>
                        </div>
                      ))}
                    </>
                  )}
                    ...
                    <>
                      ...
                      <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
                        <input type="number" value={goInput}
                          ...
                          style={{ flex: 1, padding: "14px 16px", borderRadius: 16,
                            border: `1px solid ${C.borderG}`,
                            background: "rgba(255,255,255,0.05)",
                      ...
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                        {[1, Math.floor(numPages * .25), Math.floor(numPages * .5),
                          Math.floor(numPages * .75), numPages]
                    ...
                    <>
                      ...
                      <textarea value={noteInput}
                        ...
                        style={{ width: "100%", minHeight: 110, padding: 14,
                          borderRadius: 14, border: `1px solid ${C.borderG}`,
                          background: "rgba(255,255,255,0.05)",
                          color: C.tx, fontSize: 14, fontFamily: "inherit",
                      ...
                      <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
                        ...
                        {bNotes[currentPage] && (
                          <GlowBtn label="Delete" outline onClick={() => {
                            setNoteInput("");
                            const u = { ...bNotes }; delete u[currentPage];
                            setNotes(p => ({ ...p, [bKey]: u }));
                            toast_("Note deleted");
                          }} />
                        )}
                      ...
                      {Object.keys(bNotes).length > 0 && (
                        <>
                          <div style={{ fontSize: 11, color: C.txM, fontWeight: 700,
                            textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 10 }}>
                            All Notes
                          </div>
                          {Object.entries(bNotes)
                            ...
                            .map(([pg, note]) => (
                              <div key={pg}
                                ...
                                <div dir="auto" style={{ fontSize: 12, color: C.txM,
                                  overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                  {note}
                                </div>
                              </div>
      ...
      <div style={{ flexShrink: 0, background: C.glass, backdropFilter: "blur(20px)",
        ...
        {[
          ...
          return (
            <button key={item.key}
              onClick={() => {
                if (item.key === "reader" && screen !== "reader") return;
                setTab(item.key);
              }}
              style={{ flex: 1, background: "transparent", border: "none",
                cursor: item.key === "reader" && screen !== "reader" ? "default" : "pointer",
                padding: "13px 4px 15px", fontFamily: "inherit",
                position: "relative",
                opacity: item.key === "reader" && screen !== "reader" ? 0.35 : 1,
...
function PanelTitle({ title }) {
  return (
    <div style={{ fontSize: 17, fontWeight: 800, color: C.tx, marginBottom: 18 }}>
      {title}
    </div>
  );
}
  ...
  return (
    <div style={{ flexShrink: 0, width: 110, position: "relative", cursor: "pointer" }}
      ...
      <div style={{ height: 152, borderRadius: 16,
        ...
        boxShadow: `0 0 20px hsla(${hue},60%,40%,0.25), 0 8px 32px rgba(0,0,0,0.5)`,
        display: "flex", flexDirection: "column",
        alignItems: "center", justifyContent: "center",
        padding: "12px 8px", gap: 8, marginBottom: 8,
        transition: "transform .2s, box-shadow .2s" }}
        onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-4px)"; e.currentTarget.style.boxShadow = `0 0 30px hsla(${hue},60%,40%,0.4), 0 16px 40px rgba(0,0,0,0.6)`; }}
        onMouseLeave={e => { e.currentTarget.style.transform = ""; e.currentTarget.style.boxShadow = `0 0 20px hsla(${hue},60%,40%,0.25), 0 8px 32px rgba(0,0,0,0.5)`; }}>
      ...
      <button onClick={e => { e.stopPropagation(); onRemove(); }}
        style={{ position: "absolute", top: 6, right: 6,
          ...
          width: 20, height: 20, cursor: "pointer", color: C.txM, fontSize: 11,
    ...
    <div style={{ position: "relative", animation: "fadeIn .3s ease both" }}>
      <div onClick={onOpen} style={{ cursor: "pointer" }}>
        <div style={{ aspectRatio: "2/3", borderRadius: 14,
          ...
          boxShadow: `0 0 16px hsla(${hue},50%,35%,0.2), 0 6px 24px rgba(0,0,0,0.5)`,
          display: "flex", flexDirection: "column",
          alignItems: "center", justifyContent: "center",
          padding: "10px 6px", gap: 6, marginBottom: 6,
          transition: "transform .2s, box-shadow .2s" }}
          onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-3px)"; e.currentTarget.style.boxShadow = `0 0 24px hsla(${hue},50%,35%,0.35), 0 12px 32px rgba(0,0,0,0.6)`; }}
          onMouseLeave={e => { e.currentTarget.style.transform = ""; e.currentTarget.style.boxShadow = `0 0 16px hsla(${hue},50%,35%,0.2), 0 6px 24px rgba(0,0,0,0.5)`; }}>
        ...
        {lastPage && (
          <div style={{ fontSize: 8, color: C.neon, marginTop: 1 }}>Pg {lastPage}</div>
        )}
      </div>
      <button onClick={onDelete}
        style={{ position: "absolute", top: 4, left: 4,
          background: "rgba(0,0,0,0.7)", border: "none", borderRadius: "50%",
          width: 18, height: 18, cursor: "pointer", color: C.txM, fontSize: 10,
          display: "flex", alignItems: "center", justifyContent: "center" }}>×</button>
    </div>
  );
}
